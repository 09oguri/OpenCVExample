...
public class BlobDemo {
    ...
    public static void demo() {
        ...
        for (int k = 0; k < 7; k++) {
            if (k == 0) {
                RawImage = cvLoadImage("./image/BlackBalls.jpg");
                MinArea = 250;
                ErodeCount = 0;
                DilateCount = 1;
            } else if (k == 1) {
                RawImage = cvLoadImage("./image/Shapes1.jpg");
                MinArea = 6;
                ErodeCount = 0;
                DilateCount = 1;
            } else if (k == 2) {
                RawImage = cvLoadImage("./image/Shapes2.jpg");
                MinArea = 250;
                ErodeCount = 0;
                DilateCount = 1;
            } else if (k == 3) {
                RawImage = cvLoadImage("./image/Blob1.jpg");
                MinArea = 2800;
                ErodeCount = 1;
                DilateCount = 1;
            } else if (k == 4) {
                RawImage = cvLoadImage("./image/Blob2.jpg");
                MinArea = 2800;
                ErodeCount = 1;
                DilateCount = 1;
            } else if (k == 5) {
                RawImage = cvLoadImage("./image/Blob3.jpg");
                MinArea = 2800;
                ErodeCount = 1;
                DilateCount = 1;
            } else if (k == 6) {
                RawImage = cvLoadImage("./image/Rice.jpg");
                MinArea = 30;
                ErodeCount = 2;
                DilateCount = 1;
            }
            ...
            IplImage WorkingImage = cvCreateImage(cvGetSize(BWImage),
                    IPL_DEPTH_8U, 1);
    ...
    public static void PrintGrayImage(IplImage image, String caption, int MinX,
    ...
    public static void PrintImageProperties(IplImage image) {
        ...
        int rows = mat.rows();
        int depth = mat.depth();
        System.out.println("ImageProperties for " + image + " : cols=" + cols
    ...
    // Counterclockwise small angle rotation by skewing - Does not stretch
    // border pixels
    public static IplImage SkewGrayImage(IplImage Src, double angle) // angle is
                                                                     // in
                                                                     // radians
    {
        // double radians = - Math.PI * angle / 360.0; // Half because skew is
        ...
        if (nChannels != 1) {
            System.out
                    .println("ERROR: SkewGrayImage: Require 1 channel: nChannels="
                            + nChannels);
            System.exit(1);
        }
        ...
        CvMat DstMat = cvCreateMat(DstRows, DstCols, CV_8UC1); // Type matches
                                                               // IPL_DEPTH_8U
        cvSetZero(DstMat);
        cvNot(DstMat, DstMat);
